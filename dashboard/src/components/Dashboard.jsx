...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchAppointments = async () => {
            try {
                const { data } = await axios.get("https://hms-management-backend.onrender.com/api/v1/appointment/getall", { withCredentials: true });
    ...
    const handleUpdateStatus= async(appointmentId,status)=>{
        try{
            const {data}=await axios.put(
                `https://hms-management-backend.onrender.com/api/v1/appointment/update/${appointmentId}`,
                {status},
